...
int main(int argc, char *argv[]) {
	...
	option(argc, argv);
	for (int i=0; i<10; i++) {
		if (flag.alpha==1) finalPass += passGen(alphabet, flag.pLength, 0, alphabet.length()-1);
		if (flag.alphaCaps==1) finalPass += passGen(alphabetCaps, flag.pLength, 0, alphabetCaps.length()-1);
		if (flag.specials==1) finalPass += passGen(special, flag.pLength, 0, special.length()-1);
		if (flag.nums==1) finalPass += passGen(num, flag.pLength, 0, num.length()-1);
		if (flag.hex==1) finalPass += passGen(hexa, flag.pLength, 0, hexa.length()-1);
		if (flag.vowels==1) finalPass += passGen(vowel, flag.pLength, 0, vowel.length()-1);
